...
// session-start user turn: history[0] once a session has replayed, else current.
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("keeps conversationId stable from client session headers and replays frozen msg0", () => {
    ...
    // agentContinuationId / agentTaskType / agentMode are gone from the payload —
    // Kiro answers a body carrying them with 400 REQUEST_BODY_INVALID.
  ...
  it("embeds the Claude system text in the user content and sends no top-level systemPrompt", () => {
    const out = C2K({
  ...
  it("keeps the embedded system prefix stable across turns", () => {
    const first = C2K({
    ...
    expect(prefixOf(first)).toContain("stable instruction");
    expect(first.systemPrompt).toBeUndefined();
